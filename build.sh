...
COMMIT_DATE=$(git log --format="%cd" --date=format:"%d %b %Y, %H:%M" -1 2>/dev/null || echo "unknown")
...
SWAP_TOTAL=$(awk '/SwapTotal/{printf "%.1f", $2/1024/1024}' /proc/meminfo 2>/dev/null || echo "0")
...
log_ok()  { echo -e "${green}[$(_ts)] [OK]${white} $*"; }
log_w()   { echo -e "${yellow}[$(_ts)] [!!]${white} $*"; }
log_e()   { echo -e "${red}[$(_ts)] [ERR]${white} $*"; }
...
get_kernelsu_info() {
    ...
    if [ -n "$ksu_root" ]; then
        ...
        if [ -f "$ROOTDIR/.gitmodules" ] && grep -q "KernelSU" "$ROOTDIR/.gitmodules" 2>/dev/null; then
        ...
        if git -C "$ksu_root" rev-parse --git-dir >/dev/null 2>&1; then
